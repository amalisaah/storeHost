...
const  Finance1 = (props)=> {
    ...
    return (
        <>
            ...
            <main className='font-fontRoboto fMain'>
                <div className="fHero h-[556px] text-white font-fontRoboto flex flex-col justify-center bg-[url('/images/finance/background.png')]">
                    ...
                    <div className=' z-5 mt-[113px] flex justify-center text-[36px] w-full'>
                        <NavLink to='./personal' className='rounded-l-xl w-[27%] ' style={({ isActive }) => 
                    (isActive ? {background : projStyle.col ? ( projStyle.col.color) : 'var(--bgOrange)', color:'white'} : {background: 'white', color : projStyle.col ? ( projStyle.col.color) :'var(--bgOrange)'})}><button className=' w-full h-[100px] font-fontRoboto rounded-l-xl bg-inherit text-inherit fButton' id="btn1">Personal</button></NavLink>

                        <NavLink to='./business' className='fButton rounded-r-xl w-[27%]' style={({ isActive }) => 
                    (isActive ? {background: projStyle.col ? ( projStyle.col.color) :'var(--bgOrange)', color:'white'} : {background: 'white', color : projStyle.col ? ( projStyle.col.color) :'var(--bgOrange)'})}><button className='fButton w-full h-[100px]  font-fontRoboto rounded-r-xl bg-inherit text-inherit' id="btn2">Business</button></NavLink>
            ...
            <Footer logo={edit.logo} text={edit.footText} textId='footText' tel={edit.tel} telId='tel' />
        </>
    )
};
